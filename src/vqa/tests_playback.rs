...
//! VQA playback decoder tests (first half) — basic decode, audio streaming,
//! seek/rewind, and SND0 correctness.
//!
//! See [`super::tests_playback2`] for PcmDirect variant and SND1
//! cross-chunk state continuity tests.
...
pub(super) fn build_small_vqhd(num_frames: u16, audio: Option<(u16, u8, u8)>) -> [u8; 42] {
...
pub(super) fn build_chunk(fourcc: &[u8; 4], payload: &[u8]) -> Vec<u8> {
    let mut out = Vec::new();
...
pub(super) fn build_frame_payload(codebook_value: u8) -> Vec<u8> {
...
pub(super) fn build_snd1_raw_chunk(samples: &[u8]) -> Vec<u8> {
...
pub(super) fn build_playback_vqa(with_audio: bool) -> Vec<u8> {
...
pub(super) fn build_snd1_playback_vqa() -> Vec<u8> {
