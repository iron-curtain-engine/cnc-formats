...
fn extract_mix(
    ...
    for entry in entries {
        ...
        let (out_path, relative_name, warning) =
            match resolve_output_path(&out_boundary, resolved, entry.crc, Some(file_data)) {
                Ok(path) => path,
                Err(e) => {
                    eprintln!(
        ...
        // Create any nested directories inside the validated boundary, then
        // write the file bytes.  This preserves legitimate subdirectories in
        // resolved filenames without allowing escapes.
...
/// Resolve the archive entry's extraction path inside the validated output
/// boundary.
///
/// Real filenames from `--names` are preserved when they are safe.  Unsafe
/// or malformed names fall back to a deterministic CRC-based filename so the
/// extraction still succeeds without trusting hostile path text.
fn resolve_output_path(
    out_boundary: &PathBoundary,
    resolved_name: Option<&str>,
    crc: MixCrc,
    file_data: Option<&[u8]>,
) -> Result<(StrictPath, String, Option<String>), String> {
    if let Some(name) = resolved_name {
        match validate_candidate_path(out_boundary, name) {
            Ok(path) => return Ok((path, name.to_string(), None)),
            Err(reason) => {
                let fallback = fallback_filename(crc, file_data);
                let fallback_path = validate_candidate_path(out_boundary, &fallback)?;
                let warning = format!(
                    "refusing unsafe resolved name `{name}` for 0x{:08X}: {reason}; using `{fallback}`",
                    crc.to_raw()
                );
                return Ok((fallback_path, fallback, Some(warning)));
            }
        }
    }
    ...
    let fallback = fallback_filename(crc, file_data);
    let fallback_path = validate_candidate_path(out_boundary, &fallback)?;
    Ok((fallback_path, fallback, None))
}
...
mod tests {
    ...
    fn resolve_output_path_preserves_nested_relative_name() {
        ...
        let (path, relative, warning) = resolve_output_path(
            &boundary,
    ...
    fn resolve_output_path_traversal_falls_back_to_crc_name() {
        ...
        let (path, relative, warning) =
            resolve_output_path(&boundary, Some("../../evil.shp"), crc, None).unwrap();

        assert_eq!(relative, "DEADBEEF.bin");
        assert!(warning.is_some());
        path.create_parent_dir_all().unwrap();
        path.write([0xCC]).unwrap();
