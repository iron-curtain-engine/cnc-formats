...
/// LCW compress → decompress round-trip preserves data.
///
/// Why: the LCW compressor is a new addition; this verifies it produces
/// output that the existing decompressor can correctly unpack.
#[test]
fn lcw_round_trip() {
...
/// LCW compress → decompress round-trip on all-same-byte input.
///
/// Why: a fill run is the most common LCW optimization — the compressor
/// should encode this as a single fill command.
#[test]
fn lcw_round_trip_fill() {
    let input = vec![0xAA; 200];
    let compressed = crate::lcw::compress(&input);
    // Should be much smaller than the input (one fill command + end marker).
...
/// ADPCM encode → decode round-trip produces approximately equal output.
///
/// Why: ADPCM is lossy, but for a simple ramp signal the encode/decode
/// cycle should be close to the original (within a few LSBs).
#[test]
fn adpcm_round_trip_approximate() {
    // Generate a ramp: 0, 100, 200, ..., maintaining decoder sync.
    ...
    // ADPCM is lossy — check that each sample is within reasonable tolerance.
...
// ── Lossless round-trip equality tests ────────────────────────────────────────
//
// Each test converts a synthetic asset to an intermediate format and back,
// then asserts the decoded content is byte-identical to the original.
// Direct equality is the strongest possible losslessness guarantee.

/// SHP → PNG → SHP round-trip preserves exact pixel data.
///
/// Why: SHP frames are palette-indexed bitmaps.  The RGBA intermediate
/// must quantize back to the exact same indices, so the decoded pixel
...
fn lossless_shp_png_round_trip() {
    ...
    assert_eq!(
        original_pixels, reimported_pixels,
        "SHP pixels differ after PNG round-trip"
    );
}

/// SHP → GIF → SHP round-trip preserves exact pixel data.
///
/// Why: GIF is palette-indexed like SHP, so the cycle should be lossless
/// when using the same palette for both directions.
#[test]
fn lossless_shp_gif_round_trip() {
    ...
    assert_eq!(
        original_pixels, reimported_pixels,
        "SHP pixels differ after GIF round-trip"
    );
}

/// PAL → PNG → PAL round-trip preserves exact palette bytes.
///
/// Why: the 768-byte palette file must survive the 16×16 swatch encode
/// and color-extraction decode without rounding drift.  6-bit VGA values
/// are scaled to 8-bit for PNG, then divided back — any off-by-one would
/// surface here.
#[test]
fn lossless_pal_png_round_trip() {
    ...
    assert_eq!(
        original_bytes, reimported_bytes,
        "PAL bytes differ after PNG round-trip"
    );
}

/// TMP (TD) → PNG → TMP round-trip preserves exact tile pixel data.
///
/// Why: terrain tiles are opaque palette-indexed bitmaps.  The PNG
/// intermediate must quantize back to exact indices.
#[test]
fn lossless_tmp_png_round_trip() {
    ...
    assert_eq!(
        original_tile_pixels, tmp2.tiles[0].pixels,
        "TMP tile pixels differ after PNG round-trip"
    );
}

/// WSA → PNG → WSA round-trip preserves exact frame pixel data.
///
/// Why: WSA frames are XOR-delta encoded; after decode they become raw
/// palette-indexed pixels that must survive the PNG intermediary intact.
#[test]
fn lossless_wsa_png_round_trip() {
    let pal = test_palette();
    let frame0 = vec![1u8; 4]; // 2×2, all palette index 1.
    let frame1 = vec![2u8; 4]; // 2×2, all palette index 2.
    let wsa_data =
    ...
    assert_eq!(
        original_frames, reimported_frames,
        "WSA frames differ after PNG round-trip"
    );
}

/// WSA → GIF → WSA round-trip preserves exact frame pixel data.
///
/// Why: GIF supports multiple frames and palette indexing, so the WSA
/// animation should survive the cycle without pixel drift.
#[test]
fn lossless_wsa_gif_round_trip() {
    ...
    assert_eq!(
        original_frames, reimported_frames,
        "WSA frames differ after GIF round-trip"
    );
}

/// AVI encode → decode round-trip preserves exact frame and audio data.
///
/// Why: AVI uses uncompressed BGR24 + raw PCM — zero lossy stages.  Both
/// video frames (RGBA) and audio samples must be bit-identical.
#[test]
fn lossless_avi_round_trip() {
    ...
    assert_eq!(
        decoded.frames[0], frame1,
        "AVI frame 0 differs after round-trip"
    );
    assert_eq!(
        decoded.frames[1], frame2,
        "AVI frame 1 differs after round-trip"
    );
    assert_eq!(decoded.audio, audio, "AVI audio differs after round-trip");
}

/// AUD → WAV → AUD double round-trip produces identical decoded samples.
///
/// Why: ADPCM is lossy on the first encode, but a second round-trip through
/// the same codec should be stable — the signal has already been quantized
/// to representable step values.  Raw AUD bytes may differ (header fields
/// like the IMA predictor are re-encoded fresh), so we compare the decoded
/// PCM samples, which is what actually matters for audio fidelity.
#[test]
fn lossless_aud_wav_idempotent() {
    let aud_data = build_test_aud();
    let aud = AudFile::parse(&aud_data).unwrap();

    // First round-trip: AUD → WAV → AUD → decode.
    ...
    // Second round-trip: AUD1 → WAV → AUD → decode.
    ...
    assert_eq!(
        samples1, samples2,
        "Decoded audio drifted on second WAV round-trip (codec not idempotent)"
    );
}
